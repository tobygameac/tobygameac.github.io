	...
	function createPiece(name,color) {
		...
		var piece = new THREE.Object3D();
                ...
                if (name == 'duck') {
                  ...
                  var urlDuck = '3D/duck/'+name+'.dae';

                  var geo = geometries[urlDuck].children[0].children[0];
		  
		  // mesh = new THREE.Mesh(geo.geometry, geo.material);
		  mesh = new THREE.Mesh(geo.geometry, material);

                  mesh.scale.x = mesh.scale.y = mesh.scale.z = 0.06;
		
		  mesh.rotation.y += (color == WHITE) ? -Math.PI : Math.PI;
